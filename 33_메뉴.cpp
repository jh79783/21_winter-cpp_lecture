...
// 아래의 코드는 Composite Pattern으로 구현되어 있다.
//  - Composite(복합)
//  1) MenuItem(개별 객체) / PopupMenu(복합 객체)
//  2) 복합 객체는 개별 객체와 복합 객체를 모두 보관할 수 있다.
//  => 복합 객체와 개별 객체가 공통의 부모를 가지고 있어야 한다.
//  3) 복합 객체와 개별 객체의 사용법이 동일하다. (핵심!!!)
//  => 다형성을 통해 구현하고 있다!

// MenuItem과 PopupMenu의 공통의 부모
// -> MenuItem과 PopupMenu가 제공하는 공통의 기능이 반드시 부모 클래스로부터 비롯되어야한다.
class BaseMenu
{
    std::string title;
public:
